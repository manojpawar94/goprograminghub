function SocialMedia() {
    ...
    const socialMediaLinks = metadata.map((socialMedia, index) =>
        <a href={socialMedia.link} key={index.toString()} target="_blank" data-bs-toggle="tooltip"
            data-bs-placement="bottom" className="me-3 social-media-icon">
            <em className={socialMedia.icon} role="img" aria-label={socialMedia.name}></em>
        </a>
    )
